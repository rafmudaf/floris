...
if __name__=="__main__":
    ...
    N = 400
    STEP = 25
    values = np.arange(0, N, STEP)

    wd_calc_time = np.zeros(N // STEP)
    wd_size = np.zeros(N // STEP)
    ...
    ws_calc_time = np.zeros(N // STEP)
    ws_size = np.zeros(N // STEP)
    ...
    turb_calc_time = np.zeros(N // STEP)
    turb_size = np.zeros(N // STEP)
    turbine_scaling_inputs = copy.deepcopy(sample_inputs)
    for i, v in enumerate(values):
        turbine_scaling_inputs.floris["flow_field"]["wind_directions"] = [270.0]
        turbine_scaling_inputs.floris["flow_field"]["wind_speeds"] = [8.0]
